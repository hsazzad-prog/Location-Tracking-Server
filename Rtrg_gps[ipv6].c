...
int
DecodeIPAddress(char *s, struct sockaddr_in6 *sin)
{
	int  port;
	char ip[1024];
	struct hostent *he;
	...
	case 0:
		he = gethostbyname(ip);
		if(he) {
			memcpy(&sin->sin6_addr.s6_addr, he->h_addr_list[0], 4);
		} else {
			return -1;
		}
		break;
...
char *
EncodeIPAddress(char *s, struct sockaddr_in6 *sin)
{
	int x;

	x = ntohl(sin->sin6_addr.s6_addr);
	sprintf(s, "%d.%d.%d.%d:%d", 
...
main(int argc, char *argv[])
{
	...
	int  WaddrLen,k,sum=0,qsum=0,sum1=0,qsum1=0,sum2=0,qsum2=0;
	FILE* f;
        double alfa;
	...
	if(argc < 4) {
		fprintf(stderr, "usage: Rtrg_gps [ru|rr|tu|tr] [<address>] [<port>]\n");
		exit(-1);
	}
	s = argv[2]; //store ip address
	...
	if(strcmp(argv[1], "ru") == 0) {
		...
		//printf("Rtrg_gps will receive request at this port 12345\n");
		//printf("Rtrg_gps will receive request at this address %s\n", EncodeIPAddress(s1, &addr));
		printf("Rtrg_gps will receive request at this address %s\n", inet_ntop(AF_INET6, &sin6->sin6_addr, str + 1, sizeof(str) - 1));
