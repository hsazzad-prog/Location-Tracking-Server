...
//char refer;	
	FILE *fptr = fopen("answer.txt", "r");
	

	if (fptr==NULL) perror ("Error opening file");
	else {
		fgets(referTo,100,fptr);
		puts(referTo);
		fclose(fptr);
...
void sendREQUEST(){
	...
	host= (struct hostent *) gethostbyname((char *)"192.168.1.7");
	...
	if(strcmp(argv[1], "ru") == 0) {
		...
		if(getsockname(sock, &addr, &addrLen) < 0 ) {
	       ...
	       printf("Rtrg_gps will receive request at this address %s\n", EncodeIPAddress(s1, &addr));
               for(;;){
	//part kat sini dari awal check.. sbb dia tukar cam tu je... rase part ni y silap tp still curious.. erm.. tak dpt nk check kat rumah tak de ipv6 address.........	
       ...
       //       printf("%02d %02d %02d %02d %02d %02d\n",a_macadd1, a_macadd2, a_macadd3, a_macadd4, a_macadd5,a_macadd6); 
		...
		///if(strcmp(s2,"REQUEST")==0){
			...
			sendto(sock, send_data, strlen(send_data), 0, (struct sockaddr *)&addr, sizeof(struct sockaddr));*/
			

		///	sendREQUEST();
			
			//sendREQUEST(sock, (struct sockaddr *) &addr, sizeof(struct sockaddr));
