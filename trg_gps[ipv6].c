...
BindSocket(int sock, char *s)
{
	struct sockaddr_in6 localAddr;
        //int serversock = 12345;
	//struct sockaddr_in6 localAddr;
	localAddr.sin6_family = AF_INET6;
	//localAddr.sin6_addr = htonl(s);
	localAddr.sin6_addr = in6addr_any;	
	localAddr.sin6_port = htons(servPort);
...
int  sock, rtsock, i, params[2], fd;
	...
	//struct sockaddr_in *ipaddr = &addr; //make pointer from addr var
	struct sockaddr_in6 *sin6 = &addr;
	static char str[128];
	//int			addrLen;
	//addrLen = sizeof(addr);
	
 if(strcmp(argv[1], "tu") == 0) { //is used to send from node so the MN became Transmitter +++++++++++++++++++++++++++++++
		if((sock = ConnectSocket(BindSocket(CreateUDPSocket(), ""), s)) < 0) {
			exit(-1);
		}
                inet_pton(AF_INET6, s, &(addr.sin6_addr)); // IPv6
...
//} //end for loop
printf("The position request has been sent to the gateway\n");
