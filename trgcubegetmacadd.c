		     ...
		     int		has_range)
{
  ...
  switch(event->cmd)
    {
    case SIOCGIWAP:
      printf("          Cell id %02d - Address: %s\n", state->ap_num, iw_saether_ntop(&event->u.addr, buffer));
      fprintf(f,"%s",iw_saether_ntop(&event->u.addr, buffer));
      state->ap_num++;
      strncpy(p[index_sp].mac,iw_saether_ntop(&event->u.addr, buffer),17);
p[index_sp].mac[17]='\0';
      break;
    case SIOCGIWNWID:
    ...
    case IWEVQUAL:
      ...
      fprintf(f," %d %f\n", event->u.qual.level - 0x100,pow(10,(((-event->u.qual.level + 0x100) + p0 +(rand()% 2))/pl)));
      p[index_sp].sl=event->u.qual.level - 0x100;
...
// to allow only three APs which are our APs
for(i=0;i<index_sp;i++)
{
if(strncmp(p[i].essid,"mimos",5)==0)
{
index_sp ++;
}
}
if(send(sock, &index_sp, sizeof(index_sp), 0) < 0) {
