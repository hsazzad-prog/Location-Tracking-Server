/*
 *	for compiling  gcc -o trg_gps trg_gps.c libiw.so.29
      Wireless Tools
 *
...
#include <sys/time.h>
#include "rtsock.h" //new code
#include "trg_gps.h" //new code
#include <pthread.h>
/****************************** TYPES ******************************/
//new code
char recv_data[1024];
		     ...
		     int		has_range)
{
  ...
  float pl=24.0,p0=-25.0;//where pl=10*n and p0 is inititial value at distance 1m
  ...
  switch(event->cmd)
    ...
    case SIOCGIWAP:
 	if (ind.index_sp==20)
 		ind.index_sp=0;
      printf("          Cell id %02d - Address: %s\n", state->ap_num,
	     iw_saether_ntop(&event->u.addr, buffer));
      fprintf(f,"%s",iw_saether_ntop(&event->u.addr, buffer));
      state->ap_num++;
      strncpy(p[ind.index_sp].mac,iw_saether_ntop(&event->u.addr, buffer),17);
p[ind.index_sp].mac[17]='\0';
      break;
    case SIOCGIWNWID:
	    ...
	    if((event->u.essid.flags & IW_ENCODE_INDEX) > 1)
	      printf("                    ESSID:\"%s\" [%d]\n", essid,
		     (event->u.essid.flags & IW_ENCODE_INDEX));
               ...
               {printf("                    ESSID:\"%s\"\n", essid);
               fprintf(f," %s", essid);
               strcpy(p[ind.index_sp].essid,essid);}
	  }
	else
    ...
    case IWEVQUAL:
      iw_print_stats(buffer, sizeof(buffer),
		     &event->u.qual, iw_range, has_range); //this code is used to print RSSI, LQ
  //    printf("                    Signal level:%d dBm,dist:%f\n", event->u.qual.level - 0x100,pow(10,(((-event->u.qual.level + 0x100) + p0 +(rand()% 2))/pl)));
      fprintf(f," %d %f\n", event->u.qual.level - 0x100,pow(10,(((-event->u.qual.level + 0x100) + p0 +(rand()% 2))/pl)));
      p[ind.index_sp].sl=event->u.qual.level - 0x100;
      p[ind.index_sp].dis=pow(10,(((-event->u.qual.level + 0x100) + p0 + (rand()%2))/pl));
      ind.index_sp ++;
      break;
#ifndef WE_ESSENTIAL
    case IWEVGENIE:
  ...
  while(count > 0)
    {
      ...
      /*
       * Check for Active Scan (scan with specific essid) ************************************ this is for scanning with special essid
       */
	...
	if(count < 1)
	  ...
	  memcpy(scanopt.essid, args[0], scanopt.essid_len); //scanopt.essid is name of essid that we want to scan
...
//printf(" herrrrrrrrrrr:%s\n",scanopt.essid);
  ...
  if(wrq.u.data.length)
	...
	printf(":%02X", buffer[i]);
      printf("]\n");
#endif
      printf("%-8.16s  Scan completed by TRG wirless listenner:\n", ifname);
      ...
      do
	{
	  ...
	  ret = iw_extract_event_stream(&stream, &iwe,
          ...
          //printf("event->cmd=%d\n",iwe.cmd);
	  if(ret > 0 && (iwe.cmd==35605 || iwe.cmd==35611 || iwe.cmd==35841 || iwe.cmd==35591)) //these are the event number for reading cell no,essid,ss, and mode
	    print_scanning_token(&stream, &iwe, &state,
				 &range, has_range);//print line by line but RSSI and LQ is printed is one line
	}
      while(ret > 0);
      printf("\n");
    }
...
//$$$$$$$$$$$$$$$$$$$$$$ new code for socket programming #############
int
...
//send "request" status from kphone bila klik f7.
void status(const char *stat){

strncpy(BUFFER,stat,7);
}

const char* getRefer(){
	char recv[1024];
	strcpy(recv,recv_data);	
	return recv_data;
}
...
void *waitREQUEST(){
	...
	//test(recv_data);
	//widget.newSessionRefer(&recv_data);
	//widget->
	//newSessionRefer(&recv_data);
	if((f=fopen("answer.c","w+"))==NULL)
		{
		printf("cannot open this file\n");
		}
	else{
	...
	fclose(f);
          printf("TEST RECEIVE+++++++++++++++++: %s", recv_data);
	  fflush(stdout);
...
int  sock, rtsock, i, params[2], fd;
	...
	//if(argc < 2) {
	//	fprintf(stderr, "usage: trg_gps [ru|rr|tu|tr] [<address>]\n");
	//	exit(-1);
	//} 
	s = "10.1.27.100:19797"; //nak kena tukar ip dengan port everytime nak run
	argv[1] = "tu";
	
	//s = (argc < 3)? "" : argv[2];
	if(strcmp(argv[1], "ru") == 0) { // is used to receive from node so the node became receiver
		...
		for(;;) {
			recv(sock, &p, sizeof(p), 0);
                ...
                else if(strcmp(argv[1], "tu") == 0) { //is used to send from node so the MN became Transmitter +++++++++++++++++++++++++++++++
...
/* code for sending result of gps scan */

strcpy(ind.mac,"Alice@utm-test.edu.my");

if(send(sock, &ind.index_sp, sizeof(ind.index_sp), 0) < 0) {
			    printf("send failed after packets\n");
			    DestroySocket(sock);
			    exit(0);
			        }
for(i = 0; i< ind.index_sp; i++)
    {
p[i].sl=p[i].sl; //to convert from unsigned integer byte to network byteif(send(sock, &p[i], sizeof(p), 0) < 0) {
			    printf("send failed after packets\n");
			    DestroySocket(sock);
			    exit(0);
			        }
...
if(send(sock, &BUFFER, sizeof(BUFFER), 0) < 0) {
			    ...
			    printf("send failed after packets 3\n");
			    DestroySocket(sock);
			    exit(0);
			        }
...
if(strcmp(BUFFER , "REQUEST") == 0){
	...
	iret = pthread_attr_init(&tattr);
	iret = pthread_attr_setdetachstate(&tattr, PTHREAD_CREATE_DETACHED);
	iret = pthread_create(&thread1, &tattr, waitREQUEST, NULL);

}
//} //end for loop
printf("The scanning result has been sent to the Location Server\n");
// end of sending gps scan
...
//index_sp=0;
//iw_sockets_close(skfd); /* Close the socket. */
}
//}
//  return 0;
}
...
// ni edit for dpt kn mac add...... utk send ke location server. dah berjaya.. so skrg ni try edit masuk kat coding asal.. huhu
